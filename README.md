...
public:
    ...
    MyPtr(MyPtr<T> &myptr){
        ptr_ = myptr.ptr_;https://blog.csdn.net/zhwenx3/article/details/88107428


————————————————
版权声明：本文为CSDN博主「崔楠」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/weixin_42289038/article/details/104844501
        count_ = myptr.count_;
        ++(*count_);
    }
    ...
    T getValue(){
        return *ptr_;https://blog.csdn.net/zhwenx3/article/details/88107428
...
## 计算机网络
   ...
   具有大的带宽时延乘积的网络被称为长肥网络（LongFatNetwork，即LFN），而一个运行在LFN上的TCP连接被称为长肥管道。使用长肥管道会遇到多种问题。
      1. TCP首部中窗口大小为16bit，因此窗口大小最大为65535字节，这就将发送方发送但未被确认的数据的总长度限制到了65536字节。对于LFN管道，这可能会出现所有的数据都还未到达接收方，但是发送方已        受限于窗口大小而不能继续发送的情形，这就极大的降低了网络的吞吐量。扩大窗口选项可以解决这个问题。https://blog.csdn.net/zhwenx3/article/details/88107428
...
原文链接：https://blog.csdn.net/weixin_42289038/article/details/104844501
   ...
   4. **DNS，递归/迭代查询；什么时候UDP，什么时候TCP**  
      1. DNS 查询响应报文大于 512 字节时https://blog.csdn.net/zhwenx3/article/details/88107428
...
2. **常见信号** :下面是常用到的几个信号  https://blog.csdn.net/zhwenx3/article/details/88107428
...
6. **信号的用法**：
   ...
   * 更新的`sigeaction` 设置了结构体进行封装，可以携带信号外的其他数据，具体查找资料，自己没用过。https://blog.csdn.net/zhwenx3/article/details/88107428
...
4. **总结比较**： 
   ...
   9. **socket设置为非阻塞的原因**
      listenfd如果不设置非阻塞，那么当i\o复用监听到该fd的时候如果没有事件会一直阻塞，影响其他的连接处理
      clientfd如果不将 clientfd 设置成非阻塞模式，那么一旦 epoll_wait 检测到读或者写事件返回后，接下来处理 clientfd 的读或写事件，如果对端因为 TCP 窗口太小，send 函数刚好不能将数据全部发送出去，将会造成阻塞，进而导致整个服务“卡住”。
...
## 数据库
   1. **mysql存储引擎**  
      ...
      InnoDB * 支持ACID事务（以保证数据安全）、支持数据库崩溃后自动恢复，对安全性要求高。数据和索引都是在一个tablespace的逻辑空间中  
             * 支持行级锁以及外键约束，支持写并发
             * 不存储表的总行数
   2. **索引**  ：采用B-Tree结构. 实效1. 模糊匹配 2. 使用or语法两端不是都有索引 3. 复合索引没有用左侧的索引 4. 索引进行了数学运算 5. 索引使用了函数
   3. **Redis持久化**  
